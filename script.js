...
submitInput.addEventListener('mouseover', () => {
  submitInput.style.border = '1.5px solid black'
})
...
function randomLight () {
  return Math.floor(Math.random() * 71) + 160
}
...
function btnColor (color, bg, classProp) {
  const colorBtn = document.createElement('button')
  colorBtn.classList.add('button-style')
  ...
  colorBtn.addEventListener('click', () => {
    ...
    setGrid(colRow)
    cells(colRow*colRow)
    container.firstChild.classList.add(classProp)
  })
  btnOptions.append(colorBtn)
}

const rainbowBg = 'linear-gradient(90deg, rgba(253,255,182,1) 0%, rgba(255,206,220,1) 50%, rgba(180,242,255,1) 100%)'
const yellowBg = '#FDFF94'

btnColor('rainbow', rainbowBg)
btnColor('yellow', yellowBg, 'yellow')
btnColor('pink', 'pink', 'pink')
btnColor('blue', 'lightblue', 'blue')
